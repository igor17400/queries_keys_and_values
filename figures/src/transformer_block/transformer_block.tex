...
\tikzset{
  ...
  learn/.style={draw=axiscolor, line width=1.5pt, fill=c4, rounded corners=3pt,
                ...
                font=\sffamily\large, text=axiscolor},
  shp/.style={font=\sffamily\small, text=labelcolor},
  sub/.style={font=\sffamily\small, text=labelcolor, anchor=north},
  sym/.style={font=\sffamily\LARGE, text=c1},
  eq/.style={font=\sffamily\small, text=axiscolor},
}

\newcommand{\dotmark}[1]{\fill[axiscolor] ($(#1.north west)+(0.17,-0.17)$)
    circle (0.065);}
\newcommand{\wshape}[1]{\\[1pt]{\small\color{labelcolor}#1}}
...
\node[sym] at (-0.35, 0.76) {$x$};
\node[sym] at (8.60, 0.76) {$z$};
\node[sym] at (17.30, 0.76) {$y$};
...
\node[sub] at (2.05, \ysub) {$\gamma, \beta$};
...
\node[sub] at (10.75, \ysub) {$\gamma, \beta$};
...
% ---------------------------------------------------------------- equations
\node[eq] at (4.05, -1.58)
    {$z = x + \mathrm{MultiHead}\bigl(\mathrm{LN}_1(x)\bigr)$};
\node[eq] at (13.10, -1.58)
    {$y = z + \mathrm{FFN}\bigl(\mathrm{LN}_2(z)\bigr)$};
\node[eq] at (13.10, -2.32)
    {$\mathrm{FFN}(u) = \phi(u W_1 + b_1) W_2 + b_2$};

% ---------------------------------------------------------------- legend
\draw[stream] (-0.85, -3.20) -- (0.25, -3.20);
\node[shp, anchor=west, text=axiscolor] at (0.45, -3.20)
    {residual stream, $n = 5$ tokens by $d_{\mathrm{model}} = 9$};

\node[learn, minimum width=1.0cm, minimum height=0.62cm] (lg)
    at (8.30, -3.20) {\phantom{$W$}};
\fill[axiscolor] ($(lg.north west)+(0.13,-0.13)$) circle (0.06);
\node[shp, anchor=west, text=axiscolor] at (8.95, -3.20) {learned matrix};
