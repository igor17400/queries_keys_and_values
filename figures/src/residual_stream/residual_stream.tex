...
%  continuation: dots at (16.1, 1.6), label under at (16.1, 1.00),
%     kept high so it clears both the stream and the y label.
%  left cluster is stacked, not strung out: x above the stream at
%     x = -0.85, its gloss below it, and the first width stamp pushed
%     right to x = 0.45 so nothing shares a line with anything.
%
...
%
%  Width stamps, c1: below the stream at x = 0.45 and x = 16.35, and
...
\tikzset{
  ...
  panel/.style={fill=c4, rounded corners=7pt, draw=c1, draw opacity=0.30,
                line width=0.7pt},
  wid/.style={font=\sffamily\small, text=black},
...
% ---------------------------------------------------------------- continuation
\node[font=\sffamily\LARGE, text=labelcolor] at (16.1, 1.6) {$\cdots$};
\node[font=\sffamily\small, text=black] at (16.1, 1.00)
    {blocks $3$ to $L$};

% ---------------------------------------------------------------- ends
\node[anchor=east] at (-0.85, 0.36) {{\large\textcolor{c1}{$\mathbf{x}$}}};
\node[anchor=east, text=black, font=\sffamily\small] at (-1.20, -0.36)
    {row (3) \emph{chased}};
\node[anchor=west] at (17.45, 0) {{\large\textcolor{c1}{$\mathbf{y}$}}};

\node[wid, anchor=north] at (-0.25, -0.20) {$d_{\mathrm{model}} = 9$};
\node[wid, anchor=north] at (16.35, -0.20) {$d_{\mathrm{model}} = 9$};

% ---------------------------------------------------------------- key
\draw[draw=c1, line width=1.8pt] (5.25, -1.60) -- (5.95, -1.60);
\addsym{5.60}{-1.60}
\node[key, anchor=west] at (6.10, -1.60) {$\mathbf{x} + \mathrm{Sublayer}(\mathbf{x})$};
