...
%
% The column key sits under panel (a), in the only empty region of the
% figure, on two lines at y = -2.05 and -2.80 so the two entries read as a
% pair.  It is global, not panel (a)'s: the columns mean the same thing in
% both panels, which is why it uses "word i" and not "row of MultiHead".
% Above the row boxes there is no room for headers, the subtitles at 5.80
% run straight across that band.
% ================================================================
...
% what the two columns of boxes are, said once for both panels
\node[rowbox, minimum width=0.62cm, minimum height=0.46cm] at (2.50, -2.05) {};
\node[note, anchor=west] at (2.92, -2.05) {word $i$'s vector in};
\node[outbox, minimum width=0.62cm, minimum height=0.46cm] at (2.50, -2.80) {};
\node[note, anchor=west] at (2.92, -2.80) {word $i$'s vector out};
