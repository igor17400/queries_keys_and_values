...
| `gradient_path` | The backward counterpart to `residual_stream`, drawn on the same skeleton so the reader recognises it, with the arrows running right to left. Two rows over the same three blocks. (a) *without the residual*: the stream passes straight through each sublayer box, so a single c2 backward arrow has to travel through every one of them, annotated "every sublayer reshapes it on the way". (b) *with the residual*: at each $\oplus$ the backward arrow splits in two, one copy carrying on along the stream unchanged and labelled $I$, the other going up through the sublayer and labelled $\partial\mathrm{Sublayer}/\partial x$, the two summing to the $I + \partial\mathrm{Sublayer}/\partial x$ the prose derives. Annotated so it is plain that the copy travelling the stream reaches block 1 untouched however small the sublayer terms become. Row (b) reuses the geometry of `residual_stream` exactly, including the $\oplus$ construction and both sublayers above the line. No layer normalisation anywhere: this figure sits before it is introduced. |
| `layernorm_rows` | Layer normalisation done on real numbers, so the formula stops being abstract. Two panels. (a) *the arithmetic on one row*: row **(3)** *chased* of $X$ drawn as nine cells carrying actual values with a drifted scale, the computed $\mu$ and $\sigma$ written underneath, an arrow, then the same nine cells after $(x-\mu)/\sqrt{\sigma^2+\epsilon}$, with a small bar strip above each so the collapse in scale is visible rather than merely stated. Every number must be arithmetically exact, computed and not invented. (b) *the axis*: the full $5 \times 9$ matrix with a brace on each row and that row's own $(\mu_i, \sigma_i)$ listed beside it, five different pairs, annotated "each row uses only its own nine numbers". A greyed column running down the matrix, struck through, marks the direction batch normalisation would have taken and this does not. No sum in the figure ever crosses a row boundary. Black text and 12pt to match the other section 10 figures. |
